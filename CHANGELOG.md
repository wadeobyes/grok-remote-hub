...
### Changed
- **Mobile pretty Hub UI** — system-ui (no Google Fonts / no serif fallback), card home, Working resume cards, sticky primary Send, bottom-sheet modals with a grab handle, idle image/video chrome hidden. Critical inline CSS so Tailscale/Brave never flashes raw desktop HTML. Touch ≥44px kept. Desktop (≥900px) stays compact. Static cache-bust `?v=20260905p`.
